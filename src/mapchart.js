(function () {
    ...
    //     .defer(d3.json, "http://localhost:63343/Cryptoanalytics/data/map.json")
        .defer(d3.json, "http://localhost:63343/Cryptoanalytics/data/world-50m.json")
        .defer(d3.json, "http://localhost:63343/Cryptoanalytics/data/data.json")
        .await(ready);

    var country, mapFeatures, boundaries, constituency;

    function ready(error, country, boundaries) {
        mapFeatures = topojson.feature(country, country.objects.countries).features;
        var map = mapContainer.append("g").attr("class", "countries").selectAll("path").data(mapFeatures),
